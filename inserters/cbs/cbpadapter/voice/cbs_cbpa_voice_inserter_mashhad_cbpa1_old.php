...
function onlinesessionInserter($file){
    ...
    $uniqCDRs = shell_exec("cat $file | grep -i \"the statistic for hwBillingDCCSessionNumberV1\" | sed -e \"s/ /|/g\" | awk -F \"|\" '{if($8 == \"hwEventType=11000;\") print $1substr($2,0,4),substr($13,22)}' | sed -e \"s/;//g\" | awk '{seen[$1] += $2} END {for (i in seen) print i , seen[i]}'");
    $uniqCDRs = explode("\n", $uniqCDRs);
...
$city = '/cbshome/cdr_analysis/data/cbs/cbpadapter/voice/mashhad/mcbpa1';
while(true) {
    ...
    foreach($fileList as $file){
        ...
        $logFile = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/voice/mcbpa1.log";
        $errorLog = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/voice/error_mcbpa1_voice.log";
        if (!is_dir($logFile)) {
        ...
        $sql = "INSERT INTO  cbs_cbpa_voice_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        ...
        $sql2 = "INSERT INTO  cbs_cbpa_voice_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            $sql2.= "(";
        ...
        $sql4 = "INSERT INTO  cbs_cbpa_voice_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            $sql4.= "(";
        ...
        $sql3 = "INSERT INTO  cbs_cbpa_voice_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            $sql3.= "(";
