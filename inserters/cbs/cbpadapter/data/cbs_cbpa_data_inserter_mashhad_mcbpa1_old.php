...
function onlinesessionInserter($file){
    ...
    $uniqCDRs = shell_exec("cat $file | grep -i \"the statistic for hwBillingDCCSessionNumberV1\" | sed -e \"s/ /|/g\" | awk -F \"|\" '{if($8 == \"hwEventType=11006;\") print $1substr($2,0,6),substr($13,22)}' | sed -e \"s/;//g\" | awk '{seen[$1] += $2} END {for (i in seen) print i , seen[i]}'");
    $uniqCDRs = explode("\n", $uniqCDRs);
    ...
    foreach($uniqCDRs as $uniqCDR){
        ...
        $cerrc_log_id = str_pad($cbpaId, 3, '0', STR_PAD_LEFT).str_pad($event_type, 2, '0', STR_PAD_LEFT).str_pad(strtotime($cdr_date_time), 10, '0', STR_PAD_LEFT);
        $results[] = [$cerrc_log_id, $cdr_date_time, $cbpaId, $event_type, $ONLINESESSION];      
    }
    return $results;
}
...
$city = '/cbshome/cdr_analysis/data/cbs/cbpadapter/data/mashhad/mcbpa1';
while(true) {
    ...
    foreach($fileList as $file){
        ...
        $logFile = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/data/mcbpa1.log";
        $errorLog = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/data/error_mcbpa1_data.log";
        if (!is_dir($logFile)) {
        ...
        $sql = "INSERT INTO  cbs_cbpa_data_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        ...
        ////////////////////////////////////TPS///////////////////////////////////
        $pureArrayList = tpsInserter($file);
        $sql2 = "INSERT INTO  cbs_cbpa_data_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            $sql2.= "(";
        ...
        $sql2 = rtrim($sql2, ",");
        $sql2 .= " ON DUPLICATE KEY UPDATE e_count = e_count+VALUES(e_count)";
        $start = microtime(true);
        query($sql2);
        $finish_second = microtime(true)-$start;
        
        ////////////////////////////////////Online Session///////////////////////////////////
        $pureArrayList = onlinesessionInserter($file);
        //print_r ($pureArrayList);
        //echo "\n";
        $sql4 = "INSERT INTO  cbs_cbpa_data_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            //print_r ($pureArray);
            //echo "\n";
            $sql4.= "(";
        ...
        $sql4 = rtrim($sql4, ",");
        $sql4 .= " ON DUPLICATE KEY UPDATE e_count = e_count+VALUES(e_count)";
        $start = microtime(true);
        //print_r($sql4);
        ...
        query($sql4);
        $finish_second = microtime(true)-$start;
        //echo "Insert Duration: $finish_second\n\n";
        ////////////////////////////////////Delay///////////////////////////////////
        $pureArrayList = delayInserter($file);
        $sql3 = "INSERT INTO  cbs_cbpa_data_log (cerrc_log_id, cdr_date_time, cbp_id, event_type_id, e_count) VALUES ";
        foreach($pureArrayList as $pureArray){
            $sql3.= "(";
