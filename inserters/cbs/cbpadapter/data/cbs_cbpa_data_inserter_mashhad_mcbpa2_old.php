...
function onlinesessionInserter($file){
    ...
    $uniqCDRs = shell_exec("cat $file | grep -i \"the statistic for hwBillingDCCSessionNumberV1\" | sed -e \"s/ /|/g\" | awk -F \"|\" '{if($8 == \"hwEventType=11006;\") print $1substr($2,0,4),substr($13,22)}' | sed -e \"s/;//g\" | awk '{seen[$1] += $2} END {for (i in seen) print i , seen[i]}'");
    $uniqCDRs = explode("\n", $uniqCDRs);
...
$city = '/cbshome/cdr_analysis/data/cbs/cbpadapter/data/mashhad/mcbpa2';
while(true) {        
    ...
    foreach($fileList as $file){
        ...
        $logFile = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/data/mcbpa2.log";
        $errorLog = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/cbpadapter/data/error_mcbpa12data.log";
        if (!is_dir($logFile)) {
