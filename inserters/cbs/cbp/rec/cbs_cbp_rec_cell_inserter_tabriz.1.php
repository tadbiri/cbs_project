...
//cell id 
 ...
 function cbs_cbp_rec_cell_inserter($file){
     global $cbs_cell_info,$cbs_see_rec_msc_address;
    /**
     ...
     * 
     */
    $uniqKeyMap = [0,1,2,7,8];
    $countableFieldMap = [3,4,5,6];
     global $cbs_cbp_cellid;

    $uniqCDRs = shell_exec("cat $file | awk -F\"|\" '{ print $13,$35,$42+$49,$560,$501,$51,$514,$498,$19}'");   
    $uniqCDRs = explode("\n", $uniqCDRs);
    ...
    foreach($uniqCDRs as $uniqCDR){
        ...
        if($index == -1){
            $area_code__FIELD__ = '9999';
            $cell_code__FIELD__ = '9999';
        }else{
            $cell_code__FIELD__ = $uniqCDR_Parts[4];
            if(!empty($cell_code__FIELD__)){
                $lac_id = substr($cell_code__FIELD__,5,5);
                $cell_code =substr($cell_code__FIELD__,10,10);

                $index1 = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_cell_info, ['cbs_cell_code', $cell_code__FIELD__]);
                // Check for find match.
                if($index1 == -1){
                    $cbs_cell_info = query("SELECT * FROM cbs_cell_info")->result;
                    $index1 = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_cell_info, ['cbs_cell_code', $cell_code__FIELD__]);
                    ...
                    $cbs_cell_info = newCell($lac_id, $cell_code, $cell_code__FIELD__);
                    }
                }
                $area_code__FIELD__ = $cbs_see_rec_msc_address[$index]['area_code'];    
            }
            $area_code__FIELD__ = $cbs_see_rec_msc_address[$index]['area_code'];
            $cell_code__FIELD__ = '7777';
        }
 ...
 */
define('FEED_LOG_DIR', "/cbshome/cdr_analysis/data/cbs/cbp/rec/tabriz");
