...
//cell id 
 ...
 function pkg_arab($file){
    ...
    $colsMap = [
        ...
        ['offering_code', 'string', 'offering_code'],
        ['e_count', 'int', 'e_count'],
        ];
    ...
    $uniqCDRs = shell_exec("cat $file | awk -F\"|\" '{if($503 == 501001 || $503 == 501002) print $13,$503}' | sort -k1 -k2 -n | uniq -c");
    $uniqCDRs = explode("\n", $uniqCDRs);
    ...
    foreach($uniqCDRs as $uniqCDR){
        ...
        $offering_code__FIELD__ = $uniqCDR_Parts[2];
 ...
 */
define('FEED_LOG_DIR', "/cbshome/cdr_analysis/data/cbs/cbp/mon");
...
foreach($cityList as $city){
        ...
        foreach($dateDirList as $dateDir){
            ...
            foreach($fileList as $file){
                ...
                $start = microtime(true);
                $fileProcessedRows = pkg_arab($fileTempPath);

                if(empty($fileProcessedRows->result)){ echo "is empty2 \n\n";unlink($fileTempPath);continue;}  
                ...
                for($i=0; $i<count($fileProcessedRows->colsMap); $i++){
                    ...
                    if(in_array($i, $fileProcessedRows->countableFieldMap)){
                        $statementsToUpdateOnConflict .= "$itemFieldIndex = cbs_cbp_mon_pkg.$itemFieldIndex+excluded.$itemFieldIndex,";
                    }    
                }
                ...
                $sql = "INSERT INTO cbs_cbp_mon_pkg ($columnNamesCSV) VALUES $insertCSV 
