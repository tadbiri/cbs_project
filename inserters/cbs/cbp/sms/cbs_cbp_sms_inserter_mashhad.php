 ...
 * cerrc_code
 */
$cbs_gfep_name = query("SELECT * FROM cbs_gfep_name")->result;

/**
 * cerrc_id
 * cerrc_code
 */
$cbs_cbp_sms_service_type = query("SELECT * FROM cbs_cbp_sms_service_type")->result;

$cbs_see_rec_msc_address = query("SELECT area_code,city_code FROM cbs_see_rec_msc_address")->result;
...
function cbs_cbp_sms_cell_log_inserter($file){
    global $cbs_see_rec_msc_address,$cbs_cbp_sms_service_type;
   
 
     $colsMap = [
         ['cdr_date_time', 'string', 'cdr_date_time'],
         ['city_code', 'string', 'city_code'],
         ['service_usage_type', 'string', 'service_usage_type'],
     ...
     $uniqKeyMap = [0,1,2];
     $countableFieldMap = [3,4,5];
 
     $uniqCDRs = shell_exec("cat $file | awk -F \"|\" '{ print $13,$19,$42+$49,$523,$524,$563}' | sort -k1 -k2 -k3 -k4 -k5 -n | uniq -c");
     $uniqCDRs = explode("\n", $uniqCDRs);
     ...
     foreach($uniqCDRs as $uniqCDR){
         ...
         if(count($uniqCDR_Parts) != 7){continue;}
         $e_count__FIELD__ = $uniqCDR_Parts[0];;
         $dateTime = $uniqCDR_Parts[1];
         $SERVICE_TYPE = $uniqCDR_Parts[2];
         $debit_amount = $uniqCDR_Parts[3];
         /*echo "$debit_amount\n\n";
         if(!empty($debit_amount)){
            echo "1: $e_count__FIELD__ 2: $debit_amount \n";
         }*/
         $country_code = $uniqCDR_Parts[4];
         $city_code = $uniqCDR_Parts[5];
         $free_amount = $uniqCDR_Parts[6];
            ...
            //city_code
            if($country_code == '98'){
                $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_see_rec_msc_address, ['area_code', $city_code]);
                if($index == -1){
                    $city_code__FIELD__ = 'r'.$city_code;
                }else{
                    $city_code__FIELD__ = $cbs_see_rec_msc_address[$index]['city_code'];
                }    
            }else{
                $city_code__FIELD__ = 'r'.$country_code;
            }
            ...
            // SERVICE TYPE ID
            $service_type_id = '0';
            $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_cbp_sms_service_type, ['service_type_code', $SERVICE_TYPE]);
            if($index == -1){
                $service_usage_type__FIELD__ = $SERVICE_TYPE;
            }else{
                $service_usage_type__FIELD__ = $cbs_cbp_sms_service_type[$index]['service_type_id'];
            }
...
function cbs_cbp_sms_err_log_inserter($file){
   global $cbs_cbp_name,$cbs_gfep_name,$cbs_cbp_sms_service_type; 

    $colsMap = [
        ['cdr_date_time', 'string', 'cdr_date_time'],
        ['cerrc_id', 'string', 'err_code'],
    ...
    $region_id__FIELD__ = $cbs_cbp_name[$index]['region_id'];

    $uniqCDRs = shell_exec("cat $file | awk -F \"|\" '{ print $13,$19,substr($28,1,7),$29}' | sort -k1 -k2 -k3 -k4 -n | uniq -c");
    $uniqCDRs = explode("\n", $uniqCDRs);
    ...
    foreach($uniqCDRs as $uniqCDR){
        ...
        $dateTime = $uniqCDR_Parts[1];
        $SERVICE_TYPE = $uniqCDR_Parts[2];
        $gfep_code = $uniqCDR_Parts[3];
        $cerrc_id__FIELD__ = $uniqCDR_Parts[4];
        
        //GFEP_id
        $gfep_id = '0';
        $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_gfep_name, ['gfep_code', $gfep_code]);
        $gfep_id__FIELD__ = $cbs_gfep_name[$index]['gfep_id'];
        
        // SERVICE TYPE ID
        $service_type_id = '0';
        $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_cbp_sms_service_type, ['service_type_code', $SERVICE_TYPE]);
        $service_type_id__FIELD__ = $cbs_cbp_sms_service_type[$index]['service_type_id'];
 ...
 */
define('FEED_LOG_DIR', "/cbshome/cdr_analysis/data/cbs/cbp/sms/mashhad");
...
//foreach($cityList as $city){
        ...
        foreach($dateDirList as $dateDir){
            ...
            foreach($fileList as $file){
                ...
                $start = microtime(true);
                $fileProcessedRows = cbs_cbp_sms_err_log_inserter($fileTempPath);
                $finish_second = (microtime(true)-$start)*1000;
                ...
                for($i=0; $i<count($fileProcessedRows->colsMap); $i++){
                    ...
                    if(in_array($i, $fileProcessedRows->countableFieldMap)){
                        $statementsToUpdateOnConflict .= "$itemFieldIndex = cbs_cbp_sms_err_code_log.$itemFieldIndex+excluded.$itemFieldIndex,";
                    }    
                }
                ...
                $sql = "INSERT INTO cbs_cbp_sms_err_code_log ($columnNamesCSV) VALUES $insertCSV 
                        ON CONFLICT ($columnNamesUniqKeyCSV) DO UPDATE
                        SET $statementsToUpdateOnConflict;";
                
                $finish_second = (microtime(true)-$start)*1000;
                echo "SMS_Err_mapBuildQuery: $finish_second\n";
                //print_r($sql);
                //echo "\n\n\n\n";    
            ...
            $finish_second = (microtime(true)-$start)*1000;
            echo "SMS_Err_queryinserttime: $finish_second\n";

            $finish_second1 = (microtime(true)-$start1)*1000;
            echo "SMS_Err_TotalTime--> $finish_second1\n\n";

            #####################################
            if(substr(basename($fileTempPath),0,1) === "f"){
                unlink($fileTempPath);  
                continue;
            }
            ...
            //logPrinter("\n".dateLog()."Feed file: $fileNamePure \n");
            $start = microtime(true);
            $fileProcessedRows = cbs_cbp_sms_cell_log_inserter($fileTempPath);
            $finish_second = (microtime(true)-$start)*1000;
            echo "SMS_Cell_insertertime: $finish_second\n";
            ...
            for($i=0; $i<count($fileProcessedRows->colsMap); $i++){
                ...
                // 
                if(in_array($i, $fileProcessedRows->countableFieldMap)){
                    $statementsToUpdateOnConflict .= "$itemFieldIndex = cbs_cbp_sms_cell_log.$itemFieldIndex+excluded.$itemFieldIndex,";
                }    
            }
            ...
            // Make final sql command.
            $sql = "INSERT INTO cbs_cbp_sms_cell_log ($columnNamesCSV) VALUES $insertCSV 
                    ON CONFLICT ($columnNamesUniqKeyCSV) DO UPDATE
                    SET $statementsToUpdateOnConflict;";
            
            $finish_second = (microtime(true)-$start)*1000;
            echo "SMS_Cell_mapBuildQuery: $finish_second\n";
            //print_r($sql);
            //echo "\n\n\n\n";    
            // Execute sql command and check status of it.
            $start = microtime(true);
            ...
            $finish_second = (microtime(true)-$start)*1000;
            echo "SMS_Cell_queryinserttime: $finish_second\n";

            $finish_second1 = (microtime(true)-$start1)*1000;
            echo "SMS_Cell_TotalTime--> $finish_second1\n\n";
