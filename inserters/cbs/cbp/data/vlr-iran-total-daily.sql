select t1.time,sum(t1.sub) as "Current" 
from (
...
select t1.time,t1.province_name as "Province",t2.sub as "Base",t1.sub as "Current",t1.sub-t2.sub as "Deviation",cast((t1.sub-t2.sub) as float)/NULLIF(t2.sub,0) as "Deviation%" 
from (
    select b.time,p.province_name,sum(c.subscribers_total) as "sub" 
    from (
        select distinct date_trunc('day',a.date) as "day",max(a.date) OVER (PARTITION BY date_trunc('day',a.date)) as "time" 
