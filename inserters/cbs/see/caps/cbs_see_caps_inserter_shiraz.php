...
function capsInserter($file){
    ...
    $uniqCDRs = shell_exec("cat $file | grep \"CAPS From APP =\" | sed -e \"s/ /|/g\" | awk -F \"|\" '{print \"2023\"$1$2,substr($11,0,5)}' | sed -e \"s/://g\"");
    $uniqCDRs = explode("\n", $uniqCDRs);
...
define('FEED_LOG_DIR', "/cbshome/cdr_analysis/data/cbs/cbpadapter/data/shiraz/scbpa1");
    ...
    //foreach($cityList as $city){
        ...
        //foreach($dateDirList as $dateDir){
            ...
            foreach($fileList as $file){
                ...
                $insertTupleCSV = rtrim($insertTupleCSV, ",");
                $sql = "INSERT INTO cbs_cbpa_data_log VALUES $insertTupleCSV 
                ON CONFLICT ON CONSTRAINT cbs_cbpa_data_log_uniq_cons DO UPDATE
                ...
                $sql = "INSERT INTO cbs_cbpa_data_log VALUES $insertTupleCSV 
                ON CONFLICT ON CONSTRAINT cbs_cbpa_data_log_uniq_cons DO UPDATE
                SET e_count = cbs_cbpa_data_log.e_count+excluded.e_count;
                COMMIT;";
