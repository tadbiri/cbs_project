...
function seeRecMSCInserter($file){
    ...
    $colsMap = [
        ...
        ['termination_reason_id', 'string', 'termination_reason_id'],
        ['msc_id', 'string', 'msc_id'],
        ['count','int','e_count'],
        ];
    ...
    $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_see_name, ['see_code', $seeCode]);
    $regionId = $cbs_see_name[$index]['region_id'];*/
    $uniqCDRs = shell_exec("cat $file | awk -F \"|\" '{ print $29,$76,$27}' | sort -k1 -k2 -k3  -n | uniq -c");
    $uniqCDRs = explode("\n", $uniqCDRs);
...
define('FEED_LOG_DIR', "/cbshome/cdr_analysis/data/cbs/see/rec/mashhad");
...
//
                ...
                logPrinter("\n".dateLog()."Feed file: $file \n");

                $fileProcessedRows = seeRecMSCInserter($file);
