...
function insertFailedCDR1($file){
    global $cbs_see_rec_msc_call_type, $cbs_see_rec_msc_termination_reason, $cbs_see_name, $cbs_see_rec_msc_address;
    $seeCode = shell_exec("stat $file | head -n 1 | cut -d \".\" -f 3");
    $seeCode = trim($seeCode);
    $index = TypeConvertorHelper::getIndexOfKeyValueInArray($cbs_see_name, ['see_code', $seeCode]);
    $regionId = $cbs_see_name[$index]['region_id'];
    $uniqCDRs = shell_exec("cat $file | awk -F \"|\" '{ print $29,$76,$27,$70}' | sed 's/.//14' |  sed 's/.//13' | sort -k1 -k2 -k3 -k4  -n | uniq -f1 -f2 -f3 -c");
    $uniqCDRs = explode("\n", $uniqCDRs);
    ...
    foreach($uniqCDRs as $uniqCDR){
        ...
        // Calc log_id
        $serrc_log_id = str_pad($regionId, 2, '0', STR_PAD_LEFT).str_pad($termination_reason_id, 2, '0', STR_PAD_LEFT).str_pad($msc_id, 2, '0', STR_PAD_LEFT).str_pad($call_type_id, 2, '0', STR_PAD_LEFT).str_pad(strtotime($cdr_date_time), 10, '0', STR_PAD_LEFT);
        $results[] = [$serrc_log_id, $cdr_date_time, $msc_id, $regionId, $termination_reason_id,  $call_type_id, $count];
    }
    // Clear content of file after calc.
...
$city = '/cbshome/cdr_analysis/data/cbs/see/rec/not-inserted';
while(true) {
    //foreach($cityList as $city){
        ...
        foreach($dateDirList as $dateDir){
            ...
            foreach($fileList as $file){
                ...
                $logFile = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/see/rec/not_inserted.log";
                $errorLog = "/cbshome/cdr_analysis/app/cdr_db_inserter/cbs/see/rec/error_not_inserted_rec.log";
                if (!is_dir($logFile)) {
                ...
                $sql1 = "INSERT INTO  cbs_see_rec_msc_log (serrc_log_id, cdr_date_time, msc_id, region_id, termination_reason_id, call_type_id, e_count) VALUES ";
